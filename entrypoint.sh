...
if [ -d "/tmp/repo/data" ] && [ "$(ls -A /tmp/repo/data 2>/dev/null)" ]; then
    echo "Data files found in the repository. Using them directly..."
    mkdir -p /app/data
    cp -r /tmp/repo/data/* /app/data/
    echo "Data copied successfully from repository."
else
    # Try to pull data using DVC from the cloned repository
    echo "No data files found in repository. Trying to pull data from DVC..."
    cd /tmp/repo
