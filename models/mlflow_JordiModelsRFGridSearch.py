...
with mlflow.start_run(run_name="random-forest-GridSearchCV"):
   ...
   # Log all hyperparameter results in a safe format
    # for params, mean_score in zip(grid_search.cv_results_['params'], grid_search.cv_results_['mean_test_neg_mean_squared_error']):
    #     safe_param_str = "_".join(f"{k}-{v}" for k, v in params.items())  # Convert dict to a valid string
    #     mlflow.log_metric(f"CV_MSE_{safe_param_str}", -mean_score)  # Convert negative MSE to positive
